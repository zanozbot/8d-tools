...
export async function newCommand(title: string, options: NewCommandOptions = {}): Promise<void> {
  try {
    ...
    if (options.supersede) {
      ...
      const supersedeFileName = await findReportByNumber(config.directory, supersedeNum);
      if (!supersedeFileName) {
        console.error(chalk.red(`Error: Report ${formatSequenceNumber(supersedeNum)} not found.`));
        process.exit(1);
      }

      // Add supersede link
      links.push(`- Supersedes: [${formatSequenceNumber(supersedeNum)}: ${await getReportTitle(path.join(config.directory, supersedeFileName))}](./${supersedeFileName})`);

      // Update the superseded report
      await addLinkToReport(path.join(config.directory, supersedeFileName), `Superseded by: [${formattedSequence}: ${title}](./${fileName})`);
      console.log(chalk.green(`Updated report ${formatSequenceNumber(supersedeNum)} with superseded link.`));
    }
    ...
    if (options.link) {
      ...
      if (linkParts.length !== 3) {
        console.error(chalk.red('Error: Link format should be "number:LinkType:ReverseLink"'));
      ...
      if (!linkFileName) {
        console.error(chalk.red(`Error: Report ${formatSequenceNumber(linkNum)} not found.`));
        process.exit(1);
      }
    ...
    console.log(chalk.blue(`File: ${path.relative(process.cwd(), filePath)}`));
